...
@tbe_register.register_operator("Eye", trans_bool_to_s8=False)
...
def eye(y_in__, y_out_, num_rows, num_columns=0, batch_shape=[], dtype=0, kernel_name="eye", impl_mode=""):
    ...
    origin_func_name = "eye"
    ascendc_src_dir_ex = "eye"
    ascendc_src_dir = "eye"
    ascendc_src_file = "eye.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Eye, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Eye"
...
def op_select_format(y_in__, y_out_, num_rows, num_columns=0, batch_shape=[], dtype=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, y_out_, num_rows, num_columns, batch_shape, dtype)
    result = check_op_cap("op_select_format", "Eye", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_in__, y_out_, num_rows, num_columns=0, batch_shape=[], dtype=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, y_out_, num_rows, num_columns, batch_shape, dtype)
    result = check_op_cap("get_op_specific_info", "Eye", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
