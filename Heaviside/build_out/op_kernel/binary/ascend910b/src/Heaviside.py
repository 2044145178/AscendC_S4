...
def _build_args(input_in__, values_in__, out_out_):
    __inputs__ = []
    for arg in [input_in__, values_in__]:
        if arg != None:
...
@tbe_register.register_operator("Heaviside", trans_bool_to_s8=False)
...
def heaviside(input_in__, values_in__, out_out_, kernel_name="heaviside", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, values_in__, out_out_)
    ...
    custom_all_compile_options = {'__ALLSOC__': ['--cce-auto-sync=off', '-O3']},
    ...
    origin_func_name = "heaviside"
    ascendc_src_dir_ex = "heaviside"
    ascendc_src_dir = "heaviside"
    ascendc_src_file = "heaviside.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Heaviside, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Heaviside"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[input_in__, values_in__], origin_outputs = [out_out_],\
...
def op_select_format(input_in__, values_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, values_in__, out_out_)
    result = check_op_cap("op_select_format", "Heaviside", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, values_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, values_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "Heaviside", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
