...
[[maybe_unused]] uint32_t NNOPBASE_ScatterReduce = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnScatterReduceTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "ScatterReduce";
    char inputDesc[] = {1, 1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {1, 1, 0};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(self);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(index);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(src);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, self, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, index, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, src, 2));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&dim), sizeof(int64_t), 0, kNnopbaseInt));
    NNOPBASE_ASSERT_NOTNULL_RETVAL(reduce);
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(reduce), strlen(reduce) + 1, 1, kNnopbaseString));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&includeSelf), sizeof(bool), 2, kNnopbaseBool));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddOutput(*executor, out, 0));
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "self", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "index", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "src", true));
