...
def _build_args(self_in__, index_in__, src_in__, y_out_, dim, reduce, include_self):
    __inputs__ = []
    for arg in [self_in__, index_in__, src_in__]:
        if arg != None:
    ...
    if include_self != None:
        attr = {}
        attr["name"] = "include_self"
        attr["dtype"] = "bool"
        attr["value"] = include_self
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ScatterReduce", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def scatter_reduce(self_in__, index_in__, src_in__, y_out_, dim, reduce, include_self=True, kernel_name="scatter_reduce", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, index_in__, src_in__, y_out_, dim, reduce, include_self)
    ...
    origin_func_name = "scatter_reduce"
    ascendc_src_dir_ex = "scatter_reduce"
    ascendc_src_dir = "scatter_reduce"
    ascendc_src_file = "scatter_reduce.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ScatterReduce, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ScatterReduce"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[self_in__, index_in__, src_in__], origin_outputs = [y_out_],\
...
def op_select_format(self_in__, index_in__, src_in__, y_out_, dim, reduce, include_self=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, index_in__, src_in__, y_out_, dim, reduce, include_self)
    result = check_op_cap("op_select_format", "ScatterReduce", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(self_in__, index_in__, src_in__, y_out_, dim, reduce, include_self=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, index_in__, src_in__, y_out_, dim, reduce, include_self)
    result = check_op_cap("get_op_specific_info", "ScatterReduce", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
