...
extern "C" __global__ __aicore__ void scatter_reduce(GM_ADDR self, GM_ADDR index, GM_ADDR src, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    // int singleDiff=1;
    // for(int i=tiling_data.nIndexDims-1;i>=tiling_data.dim;i--){
    //     singleDiff*=tiling_data.mmIndexDims[i];
    // }

    // if(tiling_data.mmInputDims[tiling_data.dim]>300 && tiling_data.inputSize/tiling_data.mmInputDims[tiling_data.dim]*sizeof(DTYPE_Y)/32<400){
    //     return;
    // }
